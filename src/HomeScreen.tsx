...
import { ITransaction } from './store/reducer';


interface ITypeITransaction {
  item: ITransaction
}


const HomeScreen = ({ navigation }) => {
  const { balance, transactions } = useSelector((state: RootState) => {
    return state.accountSlice
  });

  const renderItem = ({ item }: ITypeITransaction) => (
